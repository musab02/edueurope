...
// 11. Scrape Estonia (DreamApply)
async function scrapeEstonia() {
  console.log('--- Scraping Estonia (DreamApply) ---');
  let itemsCollected = [];

  try {
    console.log('[Estonia] Fetching homepage to initialize session...');
    const response = await fetch('https://estonia.dreamapply.com/', {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)'
      }
    });
    ...
    console.log('[Estonia] Sending POST to start search session...');
    const body = new URLSearchParams();
    body.append('ir', 'all');
    body.append('lang[]', 'en'); // English

    const postResponse = await fetch('https://estonia.dreamapply.com/courses/results', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Cookie': cookies,
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)',
        'Referer': 'https://estonia.dreamapply.com/courses/search',
    ...
    if (!postResponse.ok) {
      throw new Error(`[Estonia] POST failed with status: ${postResponse.status}`);
    }

    const json = await postResponse.json();
    const sessionId = json.search;
    console.log(`[Estonia] Active Session ID: ${sessionId}`);

    let html = json.results || '';
    
    // Determine total results and page count
    let totalCount = 191; // Fallback
    const countMatch = html.match(/class="value">\s*(\d+)\s*<\/div>/);
    if (countMatch) {
      totalCount = parseInt(countMatch[1]);
      console.log(`[Estonia] Found total courses count: ${totalCount}`);
    }

    const itemsPerPage = 50;
    const totalPages = Math.ceil(totalCount / itemsPerPage);
    console.log(`[Estonia] Total pages to fetch: ${totalPages}`);
    ...
    const parsePage = (pageHtml) => {
      ...
      for (let i = 1; i < segments.length; i++) {
        ...
        for (const st of smallTexts) {
          ...
          } else if (!st.includes(':') && (st.includes('full-time') || st.includes('part-time') || st.includes('year') || st.includes('month'))) {
        ...
        if (studyMode.toLowerCase() === 'full-time') {
          studyMode = 'In Person (Full-Time)';
        } else if (studyMode.toLowerCase() === 'part-time') {
          studyMode = 'In Person (Part-Time)';
        }
        ...
        itemsCollected.push({
          id: `estonia_${relativeLink.replace(/[^a-zA-Z0-9]/g, '_')}`,
          title,
          university,
          degree: stdDegree,
          location,
          ...
          studyMode,
          deadlines: [],
          link: relativeLink ? 'https://estonia.dreamapply.com' + relativeLink : '',
          logo: '',
          country: 'Estonia',
          tuitionFee,
          semesterStart
    ...
    // Parse page 1
    console.log('[Estonia] Parsing page 1...');
    parsePage(html);
    console.log(`[Estonia] Collected ${itemsCollected.length} / ${totalCount}`);

    // Fetch subsequent pages
    for (let page = 2; page <= totalPages; page++) {
      console.log(`[Estonia] Fetching page ${page}...`);
      const pageBody = new URLSearchParams();
      pageBody.append('page', page.toString());

      const pageResponse = await fetch(`https://estonia.dreamapply.com/courses/results/id/${sessionId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Cookie': cookies,
          'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)',
          'Referer': `https://estonia.dreamapply.com/courses/search/id/${sessionId}`,
          'X-Requested-With': 'XMLHttpRequest',
          'Accept': 'application/json, text/javascript, */*; q=0.01'
        },
      ...
      if (!pageResponse.ok) {
        console.error(`[Estonia] Failed to fetch page ${page}: status ${pageResponse.status}`);
        break;
      }
    ...
    console.error('[Estonia] Error during scraping:', error);
  }

  console.log(`[Estonia] Complete. Total: ${itemsCollected.length}`);
  return itemsCollected;
}
